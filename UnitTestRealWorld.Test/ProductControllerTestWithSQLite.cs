...
{
    public class ProductControllerTestWithSQLite : DatabaseProviderTest
    {
        public ProductControllerTestWithSQLite()
        {
            var connection = new SqliteConnection("DataSource=:memory:");
            connection.Open();
            SetContextOptions(new DbContextOptionsBuilder<DatabaseProviderDbContext>().UseSqlite(connection).Options);
        }
