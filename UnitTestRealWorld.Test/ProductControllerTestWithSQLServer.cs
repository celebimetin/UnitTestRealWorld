...
{
    public class ProductControllerTestWithSQLServer : DatabaseProviderTest
    {
        public ProductControllerTestWithSQLServer()
        {
            var connection = @"Data Source=(localdb)\MSSQLLocalDB;Initial Catalog=TestDB;Integrated Security=True;Connect Timeout=30;Encrypt=False;Trust Server Certificate=False;Application Intent=ReadWrite;Multi Subnet Failover=False";

            SetContextOptions(new DbContextOptionsBuilder<DatabaseProviderDbContext>().UseSqlServer(connection).Options);
        }
